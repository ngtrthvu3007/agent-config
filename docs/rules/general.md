...
`general.md` là rule nền agent đọc trước mọi thay đổi code. Trang này giúp bạn mở source thật, nhận ra pattern, và copy rule khi cần so sánh với project khác.

Nguồn: [`.claude/rules/general.md`](https://github.com/VuNguyentranThe/agent-config/blob/master/.claude/rules/general.md)

## Điểm đáng chú ý

- Phạm vi rộng nhất trong nhóm rules; dùng cho mọi thay đổi.
- Nhấn mạnh phạm vi nhỏ, pattern có sẵn, TypeScript rõ ràng, và logic dự phòng.
- Trỏ về `docs/engineering/conventions/general.md` để đọc phần giải thích dài hơn.
