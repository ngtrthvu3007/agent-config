...
`frontend.md` is the rule an agent reads when a task touches React or Next.js. This page helps you open the real source, notice the pattern, and copy the rule when comparing it with another project.

Source: [`.claude/rules/frontend.md`](https://github.com/VuNguyentranThe/agent-config/blob/master/.claude/rules/frontend.md)

## What To Notice

- Separates components, handlers, hooks, data fetching, styling, and UI states.
- Reminds the agent not to change visual design when the task is only logic or bug fixing.
- Points to `docs/engineering/conventions/frontend.md` for the full convention.
