...
`database.md` is the rule an agent reads when a task touches schema, migrations, or queries. This page helps you open the real source, notice the pattern, and copy the rule when comparing it with another project.

Source: [`.claude/rules/database.md`](https://github.com/VuNguyentranThe/agent-config/blob/master/.claude/rules/database.md)

## What To Notice

- Sets approval gates for schema and migration changes.
- Reminds the agent to protect existing data and keep queries intentional.
- Points to `docs/engineering/conventions/database.md` for the full convention.
