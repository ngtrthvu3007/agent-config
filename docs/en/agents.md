...
`AGENTS.md` is the repo's base Context file. **ChatGPT Codex** reads it directly; **Claude Code** reuses it through [`CLAUDE.md`](/en/claude); other agents can reuse the same convention when they support `AGENTS.md`.

Source: [`AGENTS.md`](https://github.com/VuNguyentranThe/agent-config/blob/master/AGENTS.md) · [Download](https://raw.githubusercontent.com/VuNguyentranThe/agent-config/master/docs/public/downloads/AGENTS.md)

## Role In The Repo

This file keeps the shared rules an agent needs before reading deeper docs. It does not replace [`rules`](/en/rules/general) or [`skills`](/en/skills/plan-feature); it routes the agent to the right place when a task needs more context.

For example, a feature change starts with `Core Rules`, moves through `Skill Routing` to pick the right workflow, then reads a specialized rule such as [Frontend Rules](/en/rules/frontend) or [Backend Rules](/en/rules/backend).

## Parts To Notice

`Purpose` makes the agent relationship explicit: ChatGPT Codex reads this file directly, while Claude Code imports it through `CLAUDE.md`.

`Core Rules` are the safety layer before code edits. Rules such as keeping scope small and avoiding unrelated files help prevent broad accidental rewrites.

`Approval Gates` lists changes that need confirmation first, such as schema, API shape, auth, payment, secrets, infrastructure, or major dependencies.

`Skill Routing` connects request types to skills, such as planning, implementation, review, QA, security, or docs.
