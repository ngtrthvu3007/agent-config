...
`review-diff` is the skill for inspecting git changes before continuing, committing, cherry-picking, or reviewing. This page helps you open the real source, notice the pattern, and copy the skill when comparing it with another project.

Source: [`.claude/skills/review-diff/SKILL.md`](https://github.com/VuNguyentranThe/agent-config/blob/master/.claude/skills/review-diff/SKILL.md)

## What To Notice

- Prioritizes version-control safety before code-quality review.
- Starts with change inventory, staged/unstaged status, and overwrite signals.
- Does not run destructive git commands without approval.
