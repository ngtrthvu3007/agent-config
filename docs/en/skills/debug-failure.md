...
`debug-failure` is the skill for bugs, failing tests, regressions, stack traces, or broken behavior. This page helps you open the real source, notice the pattern, and copy the skill when comparing it with another project.

Source: [`.claude/skills/debug-failure/SKILL.md`](https://github.com/VuNguyentranThe/agent-config/blob/master/.claude/skills/debug-failure/SKILL.md)

## What To Notice

- Starts from the provided failure instead of guessing.
- Finds root cause before fixing symptoms.
- Applies the smallest safe fix and runs relevant verification.
