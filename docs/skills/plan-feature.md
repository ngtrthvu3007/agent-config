...
`plan-feature` là skill dùng để biến yêu cầu thành kế hoạch trước khi chỉnh code. Trang này giúp bạn mở source thật, nhận ra pattern, và copy skill khi cần so sánh với project khác.

Nguồn: [`.claude/skills/plan-feature/SKILL.md`](https://github.com/VuNguyentranThe/agent-config/blob/master/.claude/skills/plan-feature/SKILL.md)

## Điểm đáng chú ý

- Không chỉnh code; tập trung vào phạm vi, bước thực hiện, acceptance criteria, kiểm tra, và rủi ro.
- Đọc `docs/specs/`, `docs/domain/`, và `docs/engineering/` trước khi lập kế hoạch.
- Nêu approval gates trước khi chạm vào schema, API, auth, payment, secrets, hạ tầng, hoặc dependency.
