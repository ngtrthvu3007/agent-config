...
`qa-test` là skill dùng để xác thực behavior thay vì chỉnh code. Trang này giúp bạn mở source thật, nhận ra pattern, và copy skill khi cần so sánh với project khác.

Nguồn: [`.claude/skills/qa-test/SKILL.md`](https://github.com/VuNguyentranThe/agent-config/blob/master/.claude/skills/qa-test/SKILL.md)

## Điểm đáng chú ý

- Lập checklist nhỏ theo happy path, edge cases, và regression risk.
- Ghi bằng chứng từ command, output, status code, hoặc bước reproduce.
- Không tự sửa lỗi nếu chưa được yêu cầu.
