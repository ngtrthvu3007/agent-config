...
`implement-feature` là skill dùng khi task đã rõ phạm vi và cần chỉnh code. Trang này giúp bạn mở source thật, nhận ra pattern, và copy skill khi cần so sánh với project khác.

Nguồn: [`.claude/skills/implement-feature/SKILL.md`](https://github.com/VuNguyentranThe/agent-config/blob/master/.claude/skills/implement-feature/SKILL.md)

## Điểm đáng chú ý

- Giữ thay đổi nhỏ và bám pattern hiện có.
- Đọc rule kỹ thuật liên quan trước khi sửa.
- Chạy kiểm tra nhỏ nhất phù hợp trước khi báo kết quả.
