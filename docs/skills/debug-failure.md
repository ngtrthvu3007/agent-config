...
`debug-failure` là skill dùng khi có bug, test fail, regression, stack trace, hoặc hành vi hỏng. Trang này giúp bạn mở source thật, nhận ra pattern, và copy skill khi cần so sánh với project khác.

Nguồn: [`.claude/skills/debug-failure/SKILL.md`](https://github.com/VuNguyentranThe/agent-config/blob/master/.claude/skills/debug-failure/SKILL.md)

## Điểm đáng chú ý

- Bắt đầu từ lỗi được cung cấp thay vì đoán.
- Tìm nguyên nhân gốc trước khi sửa triệu chứng.
- Áp dụng bản sửa nhỏ nhất và chạy kiểm tra liên quan.
