...
`review-technical` là skill dùng để review chất lượng kỹ thuật của file, module, component, service, hoặc endpoint được chỉ định. Trang này giúp bạn mở source thật, nhận ra pattern, và copy skill khi cần so sánh với project khác.

Nguồn: [`.claude/skills/review-technical/SKILL.md`](https://github.com/VuNguyentranThe/agent-config/blob/master/.claude/skills/review-technical/SKILL.md)

## Điểm đáng chú ý

- Không phụ thuộc vào git diff; phù hợp khi cần review một vùng code cụ thể.
- Tập trung vào maintainability, architecture fit, performance, TypeScript, và test quality.
- Không chỉnh file nếu chỉ được yêu cầu review.
